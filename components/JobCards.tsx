...
const CreateAppointmentModal = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const timeMap: { [key: string]: number } = {
      '6:00 am': 0, '7:00 am': 1, '8:00 am': 2, '9:00 am': 3,
      '10:00 am': 4, '11:00 am': 5, '12:00 pm': 6, '1:00 pm': 7, '2:00 pm': 8
    };

    const appointment = {
      id: Date.now().toString(),
      clientName: job.name,
      time: `${selectedTime} - ${selectedTime}`, // You'd calculate end time based on duration
      startTime: selectedTime,
      endTime: selectedTime, // Calculate properly
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-[11.75px] flex items-center justify-center z-50">
      <div className="bg-white rounded-[15px] p-6 w-96 max-w-[90vw] shadow-lg">
        <h3 className="font-semibold text-lg text-[#232529] mb-4">Create Appointment</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block font-medium text-sm text-[#232529] mb-2">Client</label>
            <input 
              type="text" 
              value={job?.name || ''} 
              disabled 
          ...
          <div>
            <label className="block font-medium text-sm text-[#232529] mb-2">Assign to</label>
            <select 
          ...
          <div>
            <label className="block font-medium text-sm text-[#232529] mb-2">Time</label>
            <select 
              value={selectedTime} 
...
export default function JobCards() {
  ...
  const handleAssignAll = () => {
    ...
    jobs.filter(job => !job.assignedMember).forEach(job => {
      setModalJob(job);
    });
  };
    ...
    <>
      <div className="w-[280px] h-full bg-white border-l border-[#EEEFF1] flex flex-col">
        ...
        <div className="flex justify-center items-center gap-2 py-3 border-b border-[#EEEFF1]">
          ...
          >
            <span className={`font-normal text-xs leading-4 flex items-center tracking-[-0.24px] truncate ${
              assignedFilter === 'Assigned' ? 'text-[#232529]' : 'text-[#667085]'
            }`}>
              Assigned
            </span>
          </button>
          ...
          >
            <span className={`font-normal text-xs leading-4 flex items-center tracking-[-0.24px] truncate ${
              assignedFilter === 'Unassigned' ? 'text-[#232529]' : 'text-[#667085]'
            }`}>
              Unassigned
            </span>
          </button>
        ...
        <div className="flex flex-col px-4 overflow-y-auto">
          {filteredJobs.map((job, index) => {
            const assignedMember = job.assignedMember ? teamMembers.find(m => m.id === job.assignedMember) : null;
